...
/**
 * 이미 서버에서 받아 온 콕 목록으로 배지 상태를 통째로 바꾼다. 거절이 실패한 뒤 콕 화면이 서버 목록을
 * 다시 불러왔을 때 그 결과를 배지에도 전달하는 데 쓴다 — 킬스위치가 꺼져 있으면 배지 조회가 나가지
 * 않아서 화면과 배지 숫자가 어긋난 채로 남기 때문이다.
 *
 * 전제조건: `cooks`가 방금 서버에서 받은 응답이다.
 *
 * 부작용: 진행 중이던 콕 조회는 응답을 버린다(더 오래된 응답이 이 결과를 덮지 못하게). 매칭 상태는
 * 건드리지 않는다. 구독자에게 알린다.
 */
