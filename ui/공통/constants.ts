 ...
 * 참가 가능한 나이의 하한.
 *
 * 가입 화면의 검증과 탐색 필터의 눈금이 같은 값을 봐야 한다. 따로 두면
 * 19세로 가입한 사람이 탐색 목록에서 영영 안 보이는 일이 생긴다 — 아무
 * 조건을 걸지 않아도 필터 하한에 걸려 사라진다.
 */
export const MIN_AGE = 19;

/**
 * 탐색 나이 눈금의 상한. 대학 축제라 이 위는 사실상 없다.
 *
 * 가입은 이 값으로 막지 않는다. 눈금은 흔한 범위를 편하게 고르라고 있는
 * 것이고, 드물게 나이가 많은 참가자를 가입 단계에서 돌려보낼 이유는 없다.
 */
export const MAX_FILTER_AGE = 35;
